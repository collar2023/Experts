...
# -*- coding: utf-8 -*-
"""
Flask服务 - 外汇信号中转站 v5.5 (SQLite 物理隔离版)
架构: 黄金版同款内核 (WAL并发 | DB锁去重 | 自动维护)
配置: 严格匹配原始映射列表
"""
...
log_file_path = os.path.join(LOG_DIR, 'mt5_forex_hub.log')
...
def init_db():
    try:
        with get_db_conn() as conn:
            ...
            conn.commit()
            logger.info("✅ SQLite (外汇版) 初始化完成 (WAL模式)")
    except Exception as e:
...
def webhook():
    try:
        ...
        if is_signal_duplicate(signal_id):
            logger.warning(f"🔁 拦截重复外汇信号: {signal_id}")
        ...
        logger.info(f"✅ 外汇信号发布: {signal_id}")
