//+------------------------------------------------------------------+
//| SuperTrend EA 日志模块 v6.1 (编译修复版)                       |
//+------------------------------------------------------------------+
#property strict

#include <Trade\AccountInfo.mqh>
// ★ 不再需要包含 EA_Parameter_Structs.mqh，因为主文件会处理
...
public:
   ...
   bool              Initialize(const SLogInputs &inputs, string fileName = "");
   void              Cleanup();
   ...
   void              LogTradeExecution(string action, ulong ticket, double price, double volume);
...
bool CLogModule::Initialize(const SLogInputs &inputs, string fileName = "")
{
   m_logLevel = inputs.logLevel;
   m_enableFileLog = inputs.enableFileLog;
   m_enableConsoleLog = inputs.enableConsoleLog;

   string symbol   = Symbol();
   string date     = TimeToString(TimeCurrent(), TIME_DATE);
   string subDir   = "Logs\\" + symbol + "\\";
   ...
   WriteInfo("EA版本: v" + inputs.eaVersion);
...
void CLogModule::LogTradeExecution(string action, ulong ticket, double price, double volume)
{
...
void CLogModule::UpdateDrawdown(double currentBalance)
{
   ...
   double dd = (m_peakBalance > 0) ? (m_peakBalance - currentBalance) / m_peakBalance * 100.0 : 0;
   if(dd > m_maxDrawdown)
   {
      m_maxDrawdown = dd;
...
{
   WriteLog(LOG_LEVEL_INFO, LOG_TYPE_SYSTEM, "========== 主要参数设置 ==========");
   WriteLog(LOG_LEVEL_INFO, LOG_TYPE_SYSTEM, "详细参数请参照EA输入面板。");
   WriteLog(LOG_LEVEL_INFO, LOG_TYPE_SYSTEM, "===============================");
}

bool InitializeLogger(const SLogInputs &inputs)
{
   if(g_Logger != NULL) delete g_Logger;
   g_Logger = new CLogModule();
   if(g_Logger == NULL) return false;
   return g_Logger.Initialize(inputs);
}
